...
print(model_name)
if not model_name:
    # 初始化模型、tokenizer和其他参数
    model_name = "hfl/chinese-roberta-wwm-ext"


file_path1= "downstream_data/financialnews_sentiment_train.csv" 
train_dataset=SentimentDataset2(file_path1)
label2id , id2label=train_dataset.getlabel2id()
num_classes = len(label2id) 

train_dataset=train_dataset[0:4000]
file_path2= "downstream_data/financialnews_sentiment_test.csv" 
test_dataset=SentimentDataset2(file_path2)[0:400]

if model_name.split("/")[-1]=="encoder_model":
    experimentname=model_name.split("/")[-2]+"_"+"financialnews_sentiment"
else:
    experimentname=model_name.split("/")[-1]+"_"+"financialnews_sentiment"
output_dir='classifier_models/'+experimentname
csv_file_path = 'sentiment2.csv'
...
data_collator = sentiment2_collator(tokenizer=tokenizer, max_length=510)
